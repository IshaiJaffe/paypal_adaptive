...
class CloseDeals(RequestHandler):
    def get(self):
        data = {
...
class CloseDeal(RequestHandler):
    def post(self,key):
        purchase = model.Purchase.get( key )
        payment = paypal.ExecutePayment(purchase.paykey,self.request.remote_addr)
        self.response.out.write("Yoofi")
