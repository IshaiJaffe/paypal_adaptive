...
class AdaptivePaymentCommand(object):
    def __init__(self,service, remote_address,data):
        self.headers = {
        ...
        self.raw_response = url_request( "%s%s" % ( settings.PAYPAL_ENDPOINT,service ), data=self.raw_request, headers=self.headers ).content()
...
class ExecutePayment( AdaptivePaymentCommand ):
    def __init__(self,paykey,remote_address):
        data = { 'payKey' : paykey, 'actionType': 'PAY' }
        super(ExecutePayment,self).__init__('ExecutePayment',remote_address, data )

class Pay( AdaptivePaymentCommand ):
  def __init__( self, amount, return_url, cancel_url, remote_address, secondary_receiver=None, ipn_url=None, shipping=False ):
    ...
    super(Pay,self).__init__('Pay',remote_address,data)
    ...
    if shipping:
      ...
      options_raw_response = url_request( "%s%s" % ( settings.PAYPAL_ENDPOINT, "SetPaymentOptions" ), data=options_raw_request, headers=self.headers ).content()
